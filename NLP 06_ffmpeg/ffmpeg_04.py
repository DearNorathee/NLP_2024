...
def extract_audio_1file(
    ...
    metadata = vt.get_metadata(video_path,"audio",language=language)
    ...
    language_list = metadata_filter['language'].tolist()
    ...
    for i, language_3_str in tqdm(enumerate(language_list),total=len(language_list)):
        ...
        for j, curr_file_ext in enumerate(file_extension_in):
            ...
            if curr_file_ext not in output_name:
                if "." not in curr_file_ext:
                    file_extension_in[j] = "." + curr_file_ext
                else:
                    file_extension_in[j] = curr_file_ext
                curr_output_name = output_name + "_" + language_2_str + file_extension_in[j]
                ...
                if result.returncode != 0:
                    print(f"Error encountered: {curr_output_name}")
                    print(result.stderr)
                
                elif result.returncode == 0:
                    print(f"Extract audio successfully: {curr_output_name}!!!")
