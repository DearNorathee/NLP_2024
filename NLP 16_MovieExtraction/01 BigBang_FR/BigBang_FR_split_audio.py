...
for season_int in range(2,12):
    ...
    input_audio_folder[season_int] = fr"H:\D_Video_Python\French\BigBang French\BigBang FR Season {season_str}\Season {season_str} Splitted Audio"
    sub_folder[season_int] = fr"H:\D_Video\BigBang French\BigBang FR Season {season_str}"

prefix_names: dict[tuple[int,int], str] = {}
...
for season in range(1,12):
    ...
    for episode in range(1,n_episode + 1):
        ...
        prefix_names[(season,episode)] = f"BigBang FR S{season_str}E{episode_str}"
...
for season in tqdm(range(2,12), desc= f"Season "):
    episode_names = ost.get_filename(input_audio_folder[season])
    n_episode = len(episode_names)
    try:
