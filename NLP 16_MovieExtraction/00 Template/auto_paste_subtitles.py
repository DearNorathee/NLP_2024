...
def paste_sub_1season_to_excel(
    ...
    bigbang_excel_book = wbx.return_as_wb(excel_path)
    
    for i in trange(start_ep_num, end_ep_num+1, colour="#FFC000"):
        episode_str = str(i).zfill(2)
        curr_episode_name = f"S06E{episode_str}"
        if curr_episode_name in wsx.sheet_names(bigbang_excel_book):
            print(f"\n{curr_episode_name} already in the sheet name. Skip for this sheet.")
        else:
            wsx.copy_sheet(bigbang_excel_book, from_sheet_name = template_sheet_name, to_sheet_names = curr_episode_name)
            ...
            curr_ws = bigbang_excel_book.sheets[curr_episode_name]
